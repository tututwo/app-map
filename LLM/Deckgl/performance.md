...
function App({ settings }: { settings: Settings }) {
  const layers = [
    new ScatterplotLayer<DataType>({
      // `filter` creates a new array every time `render` is called, even if minTime/maxTime have not changed
      data: DATA.filter((d) => d.time >= settings.minTime && d.time <= settings.maxTime),
...
So `data` has indeed changed. Do we have an entirely new collection of objects? Or did just certain fields changed in each row? Remember that changing `data` will update _all_ buffers, so if, for example, object positions have not changed, it will be a waste of time to recalculate them.
...
```ts title="Bad practice"
function render(layerVisibility: { circles: boolean; labels: boolean }) {
  const layers = [
...
const binaryData = new Float32Array([
  -122.4, 37.78, 1000, 255, 200, 0, -122.41, 37.775, 500, 200, 0, 0, -122.39, 37.8, 500, 0, 40, 200,
]);
```
...
const positionsAndColors = new Float32Array(
  POINT_CLOUD_DATA.flatMap((d) => [
    d.position[0],
    d.position[1],
    d.position[2],
