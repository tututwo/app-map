...
async function exportToPDF() {
  ...
  try {
    ...
    // The patch ensures all MapLibre canvases have preserveDrawingBuffer enabled
    console.log("MapLibre patch applied - all maps should be exportable");
    ...
    <header class="mb-6 flex items-center justify-between border-b pb-4">
      <div class="flex items-center space-x-4">
        <button
          ...
          class="flex items-center text-sm text-gray-700 hover:text-gray-900 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {#if isExporting}
